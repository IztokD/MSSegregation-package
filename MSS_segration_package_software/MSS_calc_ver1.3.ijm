...
var   Sd_calc,Sdmax_calc,Sdmin_calc,MSSLD_calc; // what to calculate (boolean)
...
var steps=0;
var sampling_factor=0.1;
var res_factor=0.75;

var total_area=0;


macro "MSS_calc_ver1.2" {
...
function mean_abs_ratio_(pon)   
{
	...
	for (a=0; a<(pon*ns1); a++) {
	    ...
	    total_wmean_abs_ratio=total_wmean_abs_ratio+getResult("weight",a)*abs(getResult("ratio",a)); // to je abs obtezenega povprecja
	    wmean_abs_ratio=total_wmean_abs_ratio/total_weight;
	//	print("total_weight:");
	//	print(total_weight);
	     total_mean_abs_ratio=total_mean_abs_ratio+abs(getResult("ratio",a)); // to je abs navadnega povprecja
...
function disply_aa()
{
	...
	for (aa=0; aa<(steps); aa++) {
		...
		setResult("mean weighted abs ratio",aa,awMean_abs[aa]); //kako izgleda tipicen subspace obtezen
  ...
  //	sampling_factor=0.1;
    ...
    sampling_factor_old=sampling_factor;
 	 ...
 	 while (err==true){
		...
		Dialog.addNumber("sampling factor: (more than 0):", sampling_factor);
...
function decision_what_to_calc()
{
	...
	while (err==true){
	 ...
	 /* Dialog.addString("Title:", title);
	  ...
	  Dialog.addCheckbox("5. I have 32-bit images, where pixel value means number of particle ( default is 8-bit binary images)", false);
	  Dialog.show();
	    ...
	    if ((MSSLD_calc==true)&&((Sd_calc==false)||(Sdmax_calc==false)||(Sdmin_calc==false)))
	    {
	    	 ...
	    	 if(dec==false)
	    	{
	    	 	...
	    	 	Dialog.create("Which assumtions to make about Sdmax and Sdmin?");
...
function calc_segregation_level(stack_ch1_bin_x,stack_ch2_bin_x)
{
	...
	total_area_calc(stack_ch1_bin_x,stack_ch2_bin_x);
	run("Clear Results");
...
function store_Sd_array()
{
	...
	{// it was assumed that Sdmax=1 =(or one of the two particles is not present in the max images)
		print("Ch1 or Ch2 Sd image is empty. Only one type of particle is prsent. Sd=1 assumed!");
		for (aa=0; aa<(steps); aa++) {
			aSd[aa]=1;
			aSd_SE[aa]=0;	
		}
...
function store_Sdmax_array()
{
	...
	if (assume_Sdmax==false)
	{
		...
		{
			print("Ch1 or Ch2 Sdmax image is empty. Only one type of particle is prsent. Sdmax=1 assumed!");
			for (aa=0; aa<(steps); aa++) {
				aSdmax[aa]=1;
				aSdmax_SE[aa]=0;	
			}
...
function store_Sdmin_array()
{
	...
	if (assume_Sdmin==false)
	{
		...
		if ((atotal_area[2]==0)||(atotal_area[1]==0)) // it was assumed that Sdmax=1 =(or one of the two particles is not present in the max images)
		{
			print("Ch1 or Ch2 Sdmin image is empty. Only one type of particle is prsent. Sdmin=1 assumed!");
			for (aa=0; aa<(steps); aa++) {
				aSdmin[aa]=1;
				aSdmin_SE[aa]=0;	
			}
...
if (Sd_calc==true)
{
	calc_segregation_level(stack_ch1_bin,stack_ch2_bin);//for image of your sample
...
if (Sdmax_calc==true)
{
	calc_segregation_level(stack_ch1_bin_Sdmax,stack_ch2_bin_Sdmax);//for simulated max segergation image
...
if (Sdmin_calc==true)
{
	calc_segregation_level(stack_ch1_bin_Sdmin,stack_ch2_bin_Sdmin);//for simulated min segergation image
