...
function open_stack() {
	...
	waitForUser("The two remaining images represent one image for particles of one type, and one for the particles of second type (background) . Press OK to continue");
...
// function description
	...
	Dialog.addMessage("Check/Adjust threshold and click OK! In red are selected particles. ");
	...
	Dialog.addCheckbox("Exclude (remove) holes in created binary images by size. ", false);
	Dialog.show();
...
function convert_image1()
{
	...
	waitForUser("Extracting the particles from image 1: Check/Adjust contrast and click OK");
...
function convert_image2()
{
	...
	auto_contr();
	run("Brightness/Contrast...");	
	waitForUser("Extracting the particles from image 2: Check/Adjust contrast and click OK");
	call("ij.plugin.frame.ThresholdAdjuster.setMethod",method);
	run("Threshold...");
	setSlice(middle);
	setAutoThreshold(input);
