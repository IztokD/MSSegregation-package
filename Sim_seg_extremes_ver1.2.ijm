...
//	rr_ch2=0.5*0.5*diameter_ch2*diameter_ch2;
	...
	print("% of theoretical occupancy by spheres of total area of "+ total_area_together+":"+effective_occupancy);
...
function selection_of_seg_extremes()
{
	 ...
	 while (err==true){
		...
		if((effective_occupancy>100)&&(min_segregation==2)&&(diameter_ch1>1)&&(diameter_ch2>1)){
			 ...
			 Dialog.create("Warning!");
			 Dialog.addMessage("Total area of simulated particles exceeds the available image space, next time you may have to choose that randomly placed particles can overlap or consider particles are disperesed in continous matrix (e.g one particle is pixel sized!)");
			 Dialog.show;
		}
		if((effective_occupancy>100)&&(min_segregation==2)&&((diameter_ch1==1)||(diameter_ch2==1))){
		
			 Dialog.create("Warning!");
			 Dialog.addMessage("Total area of simulated particles exceeds the available image space, next time you may have to choose that randomly placed particles can overlap!");
			 Dialog.show;
		}
...
function save_images()
{
	...
	name = "Log_sim_seg_extremes_ver1.0"; 
